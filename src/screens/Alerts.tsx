...
import { deleteAlert, generateTestAlert, getAlertsRaw, resolveAlert } from '@/services/alertService';
    ...
    try {
      ...
      if (res.success && res.data) {
        const mapped = res.data.map((d: any) => ({
          ...
          isTest: !!(d.isTest ?? d.__simulado ?? false),
    ...
    <ThemedView style={styles.container}>
      <SafeAreaView style={styles.safeArea}>
        ...
        {loading ? (
          ...
          <>
            <View style={{ marginBottom: Spacing.two, flexDirection: 'row', alignItems: 'center' }}>
              ...
              <Button
                title={testing ? 'Gerando alerta...' : 'Gerar Alerta de Teste'}
                onPress={async () => {
                  setTesting(true);
                  try {
                    const res = await generateTestAlert();
                    if (res.success) {
                      Alert.alert('Alerta Gerado', 'Alerta de teste criado com sucesso.');
                      await loadAlerts();
                    } else {
                      Alert.alert('Erro', res.error || 'Falha ao gerar alerta de teste.');
                    }
                  } catch (e: any) {
                    Alert.alert('Erro', e?.message || 'Erro desconhecido ao gerar alerta de teste.');
                  } finally {
                    setTesting(false);
                  }
                }}
                  ...
                  <View style={styles.row}>
                    <View style={{ flex: 1 }}>
                      <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                        ...
                        {item.isTest ? (
                          <ThemedText type="caption" themeColor="primary" style={{ marginLeft: Spacing.two }}>
                            🧪 ALERTA SIMULADO
                          </ThemedText>
                        ) : null}
                      </View>
